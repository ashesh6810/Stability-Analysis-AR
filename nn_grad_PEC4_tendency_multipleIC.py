...
f = h5py.File('/media/volume/sdb/RK4_analysis/KS_stuff/new_outputs/predicted_KS_Spectral_Loss_with_tendencyfft_PEC4step_lambda_reg_5_lead1.mat','r')
...
mynet = Net()
mynet.load_state_dict(torch.load('BNN_Spectral_Loss_with_tendencyfft_lambda_reg5_PEC4step_lead1.pt'))
mynet.eval()
mynet.cuda()
...
for k in range(0,eq_points):
    ...
    ygrad [k,:,:] = torch.autograd.functional.jacobian(PEC4step,x_torch[k,:])
...
hdf5storage.write(matfiledata, '.', path_outputs+'Amatrix_KS_PEC4step_tendency_fft_lambda_reg5_11_ICs_lead'+str(lead)+'.mat', matlab_compatible=True)
