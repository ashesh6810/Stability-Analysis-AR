...
f = h5py.File('/home/exouser/RK4_analysis/KS_stuff/new_outputs/predicted_one_layer_KS_Eulerstep_lead1.mat','r')
...
count=0
for k in (np.array([ int(1),  int(100), int(500), int(1000), int(3000), int(5000), int(7000), int(7000), int(10000), int(15000), int(19000)])):
...
mynet = Net()
mynet.load_state_dict(torch.load('BNN_one_layer_Eulerstep_lead1.pt'))
mynet.eval()
mynet.cuda()
...
for k in range(0,eq_points):
    ...
    ygrad [k,:,:] = torch.autograd.functional.jacobian(Eulerstep,x_torch[k,:])
...
matfiledata = {}
matfiledata[u'A_matrix_Euler'] = ygrad
hdf5storage.write(matfiledata, '.', path_outputs+'Amatrix_KS_one_layer_Eulerstep_11_ICs_lead'+str(lead)+'.mat', matlab_compatible=True)
