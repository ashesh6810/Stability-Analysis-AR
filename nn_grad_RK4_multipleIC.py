...
f = h5py.File('/home/exouser/RK4_analysis/KS_stuff/new_outputs/predicted_one_layer_KS_RK4step_lead1.mat','r')
...
mynet = Net()
mynet.load_state_dict(torch.load('BNN_one_layer_RK4step_lead1.pt'))
mynet.eval()
mynet.cuda()
...
for k in range(0,eq_points):
    ...
    ygrad [k,:,:] = torch.autograd.functional.jacobian(RK4step,x_torch[k,:])
...
matfiledata = {}
matfiledata[u'A_matrix_RK4'] = ygrad
hdf5storage.write(matfiledata, '.', path_outputs+'Amatrix_KS_one_layer_RK4step_11_ICs_lead'+str(lead)+'.mat', matlab_compatible=True)
