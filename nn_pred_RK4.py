...
#      epoch_loss=0
      for step in range(0,trainN,batch_size):
        ...
        optimizer.zero_grad()
        outputs = RK4step(mynet,input_batch)
...
torch.save(mynet.state_dict(),'BNN_one_layer_RK4step_lead'+str(lead)+'.pt') 
...
for k in range(0,M):
    ...
    if (k==0):
        ...
        out = RK4step(mynet,input_test_torch[0,:])
        pred [k,:] = out.detach().cpu().numpy()

    else:

        out = RK4step(mynet,torch.from_numpy(pred[k-1,:]).float().cuda())
...
hdf5storage.write(matfiledata, '.', path_outputs+'predicted_one_layer_KS_RK4step_lead'+str(lead)+'.mat', matlab_compatible=True)
