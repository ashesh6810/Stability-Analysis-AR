...
torch.save(mynet.state_dict(),'BNN_FNO_directstep_lead'+str(lead)+'.pt') 
...
for k in range(0,M):
    ...
    if (k==0):
        ...
        out = directstep(mynet,torch.reshape(input_test_torch[0,:],(1,input_size,1)))
        pred [k,:] = torch.reshape(out,(1,input_size)).detach().cpu().numpy()

    else:

        out = directstep(mynet,torch.reshape(torch.from_numpy(pred[k-1,:]),(1,input_size,1)).float().cuda())
...
hdf5storage.write(matfiledata, '.', path_outputs+'predicted_FNO_KS_directstep_lead'+str(lead)+'.mat', matlab_compatible=True)
