...
def lyap_loss (output, output2, target,tendency):
   loss1 = torch.mean((output-target)**2)
   ...
   my_batch = target.size()[0]
   print('batch_size inside loss',my_batch)
   L_abs_mean = torch.zeros([1]).float().cuda()


   for k in range(0,my_batch):

    net_grad = torch.autograd.functional.jacobian(PEC4step,output2[k].flatten())
#    print('grad shape',net_grad.shape)
    L= torch.linalg.eigvals(net_grad)
    L_abs_mean = L_abs_mean+torch.mean(torch.max(torch.abs(L)))

   L_abs_mean=L_abs_mean/my_batch

   return  loss + lamda_reg*torch.nn.functional.relu(L_abs_mean-0.98)
...
#      epoch_loss=0
 ...
 #       print('shape of input_batch',input_batch.shape)
        ...
        optimizer.zero_grad()
        outputs = PEC4step(input_batch)
        outputs2 = PEC4step(outputs)
        loss = lyap_loss(outputs,outputs2,label_batch,du_label_batch)
...
torch.save(mynet.state_dict(),'BNN_Lyap_Loss_with_tendencyfft_'+'lambda_reg'+str(lamda_reg)+'_PEC4step_lead'+str(lead)+'.pt') 
...
for k in range(0,M):
    ...
    if (k==0):
        ...
        out = PEC4step(input_test_torch[0,:])
        pred [k,:] = out.detach().cpu().numpy()

    else:

        out = PEC4step(torch.from_numpy(pred[k-1,:]).float().cuda())
...
hdf5storage.write(matfiledata, '.', path_outputs+'predicted_KS_Lyap_Loss_with_tendencyfft_PEC4step_'+'lambda_reg_'+str(lamda_reg)+'_lead'+str(lead)+'.mat', matlab_compatible=True)
